...
def run_backfill():
    ...
    # 3. Create the progress bar with a custom bar format matching your layout
    progress_bar = tqdm(
    ...
    for num in progress_bar:
        ...
        try:
            ...
            # Use official transcript if available, or write a clean starting placeholder
            official_transcript = comic_data.get("transcript", "").strip()
            extracted_text = official_transcript if official_transcript else f"Sample transcript or OCR output for comic #{num}"
